...
- (void)buildUI {
    ...
    for (int i = 0; i < number; i++) {
        ...
        NSString *title = [self.delegate titleOfPagers:self indexOfPagers:i];
        ...
        line.frame = CGRectMake(itemButton.right, 0, 2, self.tabFrameHeight);
        if (self.showVerticalLine && i < number-1) {
...
- (void)selectTabWithIndex:(NSUInteger)index animate:(BOOL)isAnimate {
    ...
    void(^moveSelectedLine)(void) = ^(void) {
        ...
        self.selectedLineOffsetXBeforeMoving = self.selectedLine.left;
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView {
    if (scrollView == self.bodyScrollView) {
        [self selectTabWithIndex:(int)scrollView.contentOffset.x/self.width animate:YES];
        _isUseDragging = YES;
        _isEndDecelerating = YES;
        _continueDraggingNumber = 0;
    }
...
- (CGFloat)selectedLineWidth {
    if (!_selectedLineWidth) {
        self.selectedLineWidth = self.width/2;//42
    }
...
- (UIScrollView*)bodyScrollView {
    if (!_bodyScrollView) {
        self.bodyScrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(0, self.height, self.width, self.containerView.frame.size.height- self.height)];
